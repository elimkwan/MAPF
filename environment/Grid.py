...
class Grid():
    ...
    def next(self, node, t):
        ...
        return list(zip(map(tuple,next_pos), np.ones(next_pos.shape[0])*t))
    ...
    def getOptimiserCost(self, solution, exp):
        penality = self.get_penality_cost(solution, exp.start_locations, exp.end_locations)
        ...
        cost_conwait = self.exp.getWaiting(paths = solution, grid=True)
    ...
    def get_penality_cost(self, solution, start_loc, end_loc):
        aggr = 0
