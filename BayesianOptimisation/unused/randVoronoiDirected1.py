...
def generate_data(simulator = None, dataset = 1, seed = 0):

    np.random.seed(seed) 
    cutoff_thres, num_probabilities = simulator.get_cutoff()

    x_path = "./data/Random/Dataset" + str(dataset) + "/Seed"+ str(seed) + "/x_path.csv"
    y_path = "./data/Random/Dataset" + str(dataset) + "/Seed"+ str(seed) + "/y_path.csv"
        
    for i in range(0,1,1):
        unused_ub = 1
        subgraph_ub = constant.MAX_SUBGRAPH
...
def random_voronoi_directed(exp, dataset = 1, seed = 0):
    ...
    prob, cost = generate_data(simulator = simulateObj, dataset = dataset, seed = seed)
