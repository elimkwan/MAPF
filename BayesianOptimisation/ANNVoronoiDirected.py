...
def generate_data(simulator = None, iterations = 1):
    ...
    for i in range(iterations):
        ...
        init_probability[-3] = (1-(-1)) * np.random.random_sample() + (-1) # m
        init_probability[-2] = (0.6536-(0.3464)) * np.random.random_sample() + (0.3464) # c
        print("m",init_probability[-3], "c", init_probability[-2])
